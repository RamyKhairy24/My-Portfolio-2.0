...
export const ProjectCard = ({ title, description, tags, link, github, previewImage, previewVideo }: ProjectProps) => {
    ...
    <>
      ...
      <div className="group flex flex-col mb-20 last:mb-0">
        <div className="mb-8">
          ...
          <h3 className="text-5xl md:text-7xl font-serif italic mb-6 tracking-tight leading-none">{title}</h3>
          <p className="text-brand-cream/60 max-w-2xl text-lg leading-relaxed mb-8">
            {description}
          </p>
          <div className="flex flex-wrap gap-4">
            {tags.map(tag => (
              <span key={tag} className="px-3 py-1 bg-brand-cream/5 border border-brand-cream/10 text-[10px] uppercase tracking-[0.2em] text-brand-cream/60">
                {tag}
              </span>
            ))}
          </div>
...
export const CertificationCard = ({ title, issuer, date, link }: { title: string, issuer: string, date: string, link: string }) => {
  const [isActivating, setIsActivating] = useState(false);
    ...
    <>
      ...
      <div className="group flex flex-col">
        <div className="mb-4">
          ...
          <p className="text-xs uppercase tracking-widest text-brand-cream/50">{issuer} — {date}</p>
        </div>
        ...
        >
          ...
          <img 
            src={`https://picsum.photos/seed/${title}/800/600?blur=2`}
            alt={title}
            referrerPolicy="no-referrer"
